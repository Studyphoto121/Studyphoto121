...
if game.PlaceId == 8065776277 and L_1_ then
	...
	if not isfolder("incap_configs") then
		makefolder("incap_configs")
	end;
	if not isfolder("incap_configs/democfg") then
		makefolder("incap_configs/democfg")
	end;
	function L_177_:saveConfig()
		...
		writefile("incap_configs/democfg/" .. L_614_ .. ".cfg", game:GetService("HttpService"):JSONEncode(L_615_))
		L_177_:refreshConfigs()
	end;
	function L_177_:loadConfig()
		local L_618_ = L_177_.flags["selected_config"]
		if not isfile("incap_configs/democfg/" .. L_618_ .. ".cfg") then
			return
		end;
		local L_619_ = game:GetService("HttpService"):JSONDecode(readfile("incap_configs/democfg/" .. L_618_ .. ".cfg"))
	...
	function L_177_:refreshConfigs()
		local L_622_ = {}
		for L_623_forvar0, L_624_forvar1 in next, listfiles("incap_configs/democfg") do
	...
	function L_177_:deleteConfig()
		if isfile("incap_configs/democfg/" .. L_177_.flags["selected_config"] .. ".cfg") then
			delfile("incap_configs/democfg/" .. L_177_.flags["selected_config"] .. ".cfg")
			L_177_:refreshConfigs()
		end
	end;
	...
	L_174_.wait:Destroy()
	for L_703_forvar0, L_704_forvar1 in pairs(listfiles("incap_configs/demos/")) do
...
local L_5_ = {
	...
	tabSize = 0,
	theme = {},
	foldername = "incap_free/configs/cb",
	fileext = ".txt"
}
...
function L_5_:Init()
	...
	spawn(function()
		while L_5_ do
			...
			if isfolder("incap_configs/scripts") then
				local L_928_ = {}
				for L_929_forvar0, L_930_forvar1 in next, listfiles("incap_configs/scripts") do
					local L_931_ = L_930_forvar1:match('incap_configs/scripts\\([%w%s%-]*)%.%a?')
				...
				makefolder("incap_configs/scripts")
			end
		end
	end)
...
onKill.Name = "onKill"
if not isfolder("incap_configs/tempimages") then
	makefolder("incap_configs/tempimages")
end;
if not isfolder("incap_configs/demos") then
	makefolder("incap_configs/demos")
end;
local L_61_ = -3846999;
local L_62_ = 8532252;
local L_63_ = -1162714;
	...
	local L_959_, L_960_ = pcall(function()
		...
		for L_972_forvar0, L_973_forvar1 in next, L_956_arg2 do
			if L_973_forvar1:match("^%w+://([^/]+)") then
				...
				if L_974_.Success then
					local L_975_ = "incap_configs/tempimages/" .. L_972_forvar0 .. L_955_arg1 .. L_954_arg0 .. ".png"
		...
		if L_957_arg3:match("^%w+://([^/]+)") then
			...
			if L_976_.Success then
				local L_977_ = "incap_configs/tempimages/" .. "image" .. L_955_arg1 .. L_954_arg0 .. ".png"
...
if not isfile("incap_configs/customskins.txt") then
	writefile("incap_configs/customskins.txt", [[
[
...
if isfile("incap_configs/customskins.txt") then
	succes, reason = pcall(function()
		local L_983_ = readfile("incap_configs/customskins.txt")
	...
	if not succes then
		warn("[incap] could not load custom skins! reason: " .. reason)
	end
end;
...
spawn(function()
	...
	oldNamecall = hookmetamethod(game, "__namecall", LPH_NO_VIRTUALIZE(function(L_1236_arg0, ...)
		...
		if L_1238_ == "GetService" and L_1237_[1] == "incap" then
			local L_1240_ = {
				HitEvent = onHit,
				KillEvent = onKill,
				Chat = chat,
...
L_135_.Parent = L_24_;
local L_136_, L_137_ = {}, {}
if not isfile("incap_configs/custommodels.txt") then
	writefile("incap_configs/custommodels.txt", [[
[
...
local L_138_, L_139_ = pcall(function()
	local L_1499_ = readfile("incap_configs/custommodels.txt")
...
if not L_138_ then
	warn("[incap] could not load custom models! reason: " .. L_139_)
end;
...
L_134_:AddButton({
	...
	callback = function()
		local L_1514_ = readfile("incap_configs/custommodels.txt")
...
L_134_:AddButton({
	...
	callback = function()
		...
		if L_133_:Show() then
			if isfile("incap_configs/skins.txt") then
				local L_1522_ = game:GetService"HttpService":JSONDecode(readfile("incap_configs/skins.txt"))
			...
			writefile("incap_configs/skins.txt", game:GetService"HttpService":JSONEncode(L_1521_))
		end
	end
})
L_134_:AddButton({
	...
	callback = function()
		...
		if L_133_:Show() then
			if isfile("incap_configs/skins.txt") then
				local L_1527_ = game:GetService"HttpService":JSONDecode(readfile("incap_configs/skins.txt"))
	...
	L_1564_:AddButton({
		...
		callback = function()
			...
			if L_1560_ then
				...
				if isfolder("incap_configs/demos/" .. L_1563_) then
					delfolder("incap_configs/demos/" .. L_1563_)
					makefolder("incap_configs/demos/" .. L_1563_)
				else
					makefolder("incap_configs/demos/" .. L_1563_)
				end;
				for L_1627_forvar0, L_1628_forvar1 in next, L_1555_ do
					...
					writefile("incap_configs/demos/" .. L_1563_ .. "/" .. tostring(L_1627_forvar0) .. ".tick", L_1629_)
				...
				writefile("incap_configs/demos/" .. L_1563_ .. "/ver.tick", "1")
				writefile("incap_configs/demos/" .. L_1563_ .. "/map.tick", game:service("HttpService"):JSONEncode(L_1625_))
			end;
...
ScriptSection:AddButton({
	text = "Run Script",
	callback = function()
		for L_1633_forvar0, L_1634_forvar1 in next, listfiles("incap_configs/scripts") do
			if not table.find(L_140_, L_1634_forvar1) and L_1634_forvar1:split(".")[1] == "incap_configs/scripts\\" .. library_flags["scriptlist"] then
...
spawn(function()
	wait(5)
	if not isfolder("incap_configs/autoexec") then
		makefolder("incap_configs/autoexec")
	end;
	for L_1635_forvar0, L_1636_forvar1 in next, listfiles("incap_configs/autoexec") do
...
if not isfile("incap_configs/chatspam.txt") then
	writefile("incap_configs/chatspam.txt", "Message1\nMessage2\nMessage3\nMessage4\nMessage5")
end;
...
if isfile("incap_configs/temp.tmp") then
	L_5_:LoadConfig(readfile("incap_configs/temp.tmp"))
	delfile("incap_configs/temp.tmp")
end;
L_32_.OnTeleport:Connect(function(L_1679_arg0)
end)
L_30_.PlayerRemoving:Connect(function(L_1680_arg0)
	if L_1680_arg0 == L_32_ then
		for L_1681_forvar0, L_1682_forvar1 in next, listfiles("incap_configs/tempimages") do
...
L_28_.RenderStepped:Connect(function(L_1691_arg0)
	...
	if library_flags["Chat Spammer"] then
		...
		if chatspamDebounce == 175 then
			...
			if L_1721_ == "Custom" and isfile("incap_configs/chatspam.txt") then
				local L_1722_ = readfile("incap_configs/chatspam.txt")
	...
	if library_flags["Watermark"] then
		...
		L_116_.Text = " incap | " .. L_1759_ .. ":" .. L_1758_ .. ":" .. L_1757_ .. " | " .. L_32_.Name;
...
if game.PlaceId == 8065776277 and L_1_ then
	...
	function L_179_:saveConfig()
		...
		writefile("incap_configs/democfg/" .. L_618_ .. ".cfg", game:GetService("HttpService"):JSONEncode(L_619_))
		L_179_:refreshConfigs()
	end;
	function L_179_:loadConfig()
		local L_622_ = L_179_.flags["selected_config"]
		if not isfile("incap_configs/democfg/" .. L_622_ .. ".cfg") then
			return
		end;
		local L_623_ = game:GetService("HttpService"):JSONDecode(readfile("incap_configs/democfg/" .. L_622_ .. ".cfg"))
	...
	function L_179_:refreshConfigs()
		local L_626_ = {}
		for L_627_forvar0, L_628_forvar1 in next, listfiles("incap_configs/democfg") do
	...
	function L_179_:deleteConfig()
		if isfile("incap_configs/democfg/" .. L_179_.flags["selected_config"] .. ".cfg") then
			delfile("incap_configs/democfg/" .. L_179_.flags["selected_config"] .. ".cfg")
			L_179_:refreshConfigs()
		end
	end;
	...
	L_176_.wait.TextLabel.Text = "Loading " .. tostring(# listfiles("incap_configs/demos/")) .. " demos..."
	wait()
	for L_707_forvar0, L_708_forvar1 in pairs(listfiles("incap_configs/demos/")) do
...
function L_5_:Init()
	...
	spawn(function()
		while L_5_ do
			...
			if isfolder("incap_configs/scripts") then
				local L_936_ = {}
				for L_937_forvar0, L_938_forvar1 in next, listfiles("incap_configs/scripts") do
					local L_939_ = L_938_forvar1:match('incap_configs/scripts\\([%w%s%-]*)%.%a?')
	...
	local L_967_, L_968_ = pcall(function()
		...
		for L_980_forvar0, L_981_forvar1 in next, L_964_arg2 do
			if L_981_forvar1:match("^%w+://([^/]+)") then
				...
				if L_982_.Success then
					local L_983_ = "incap_configs/tempimages/" .. L_980_forvar0 .. L_963_arg1 .. L_962_arg0 .. ".png"
		...
		if L_965_arg3:match("^%w+://([^/]+)") then
			...
			if L_984_.Success then
				local L_985_ = "incap_configs/tempimages/" .. "image" .. L_963_arg1 .. L_962_arg0 .. ".png"
...
if isfile("incap_configs/customskins.txt") then
	succes, reason = pcall(function()
		local L_991_ = readfile("incap_configs/customskins.txt")
...
spawn(function()
	...
	oldNamecall = hookmetamethod(game, "__namecall", LPH_NO_VIRTUALIZE(function(L_1244_arg0, ...)
		...
		if L_1246_ == "GetService" and L_1245_[1] == "incap" then
			local L_1248_ = {
				HitEvent = onHit,
				KillEvent = onKill,
				Chat = chat,
...
local L_119_ = {
	Standard = {
		...
		"Cant Win U Should Rq",
		"nigger cant win incap! nigger cant win incap!"
	},
	Emojis = {
		"💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿😘 😎 🤓",
		"💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿😘 😎 🤓",
		"💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿 😘 😎 🤓 💀 low rank is dead😭 😏 👽 👍🏿😘 😎 🤓"
	}
}
...
local L_138_, L_139_ = pcall(function()
	local L_1507_ = readfile("incap_configs/custommodels.txt")
...
L_134_:AddButton({
	text = "Refresh Custom Models",
	callback = function()
		local L_1522_ = readfile("incap_configs/custommodels.txt")
...
L_134_:AddButton({
	...
	callback = function()
		...
		if L_133_:Show() then
			if isfile("incap_configs/skins.txt") then
				local L_1530_ = game:GetService"HttpService":JSONDecode(readfile("incap_configs/skins.txt"))
			...
			writefile("incap_configs/skins.txt", game:GetService"HttpService":JSONEncode(L_1529_))
		end
	end
})
L_134_:AddButton({
	...
	callback = function()
		...
		if L_133_:Show() then
			if isfile("incap_configs/skins.txt") then
				local L_1535_ = game:GetService"HttpService":JSONDecode(readfile("incap_configs/skins.txt"))
	...
	L_1572_:AddButton({
		...
		callback = function()
			...
			if L_1568_ then
				...
				if isfolder("incap_configs/demos/" .. L_1571_) then
					delfolder("incap_configs/demos/" .. L_1571_)
					makefolder("incap_configs/demos/" .. L_1571_)
				else
					makefolder("incap_configs/demos/" .. L_1571_)
				end;
				for L_1635_forvar0, L_1636_forvar1 in next, L_1563_ do
					...
					writefile("incap_configs/demos/" .. L_1571_ .. "/" .. tostring(L_1635_forvar0) .. ".tick", L_1637_)
				...
				writefile("incap_configs/demos/" .. L_1571_ .. "/ver.tick", "1")
				writefile("incap_configs/demos/" .. L_1571_ .. "/map.tick", game:service("HttpService"):JSONEncode(L_1633_))
			end;
...
ScriptSection:AddButton({
	text = "Run Script",
	callback = function()
		for L_1641_forvar0, L_1642_forvar1 in next, listfiles("incap_configs/scripts") do
			if not table.find(L_141_, L_1642_forvar1) and L_1642_forvar1:split(".")[1] == "incap_configs/scripts\\" .. library_flags["scriptlist"] then
...
spawn(function()
	...
	for L_1643_forvar0, L_1644_forvar1 in next, listfiles("incap_configs/autoexec") do
...
L_30_.PlayerRemoving:Connect(function(L_1688_arg0)
	if L_1688_arg0 == L_32_ then
		for L_1689_forvar0, L_1690_forvar1 in next, listfiles("incap_configs/tempimages") do
...
L_28_.RenderStepped:Connect(function(L_1699_arg0)
	...
	if library_flags["Chat Spammer"] then
		...
		if chatspamDebounce == 175 then
			...
			if L_1729_ == "Custom" and isfile("incap_configs/chatspam.txt") then
				local L_1730_ = readfile("incap_configs/chatspam.txt")
	...
	if library_flags["Watermark"] then
		...
		L_116_.Text = " incap | " .. L_1767_ .. ":" .. L_1766_ .. ":" .. L_1765_ .. " | " .. L_32_.Name;
		if L_116_.TextBounds.X > 210 then
			L_116_.TextSize = L_116_.TextSize - 1
		end
	end;
